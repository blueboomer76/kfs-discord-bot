	...
	if (bot.phoneVars.channels.indexOf(message.channel.id) == -1) {
		...
		if (bot.phoneVars.channels.length == 1) {
			...
			if (bot.phoneVars.channels.length == 2) {
				...
				bot.channels.get(bot.phoneVars.channels.shift().toString()).send("☎️ Someone else is now using the phone...");
			}
		...
		if (bot.phoneVars.channels.length == 1) {
			...
			if (message.channel.id == bot.phoneVars.channels[0].toString()) {affected = 1};
			bot.channels.get(bot.phoneVars.channels[affected].toString()).send("☎ " + phoneMsg);
		}
